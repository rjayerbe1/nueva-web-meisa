...
export function BrochuresTable({ brochures }: BrochuresTableProps) {
  ...
  const getCategoryColor = (key: CategoriaEnum) => {
    switch (key) {
      ...
      case 'INDUSTRIAL': return 'bg-blue-100 text-blue-800 border-blue-200'
      case 'PUENTES': return 'bg-green-100 text-green-800 border-green-200'
      case 'INFRAESTRUCTURA_URBANA': return 'bg-teal-100 text-teal-800 border-teal-200'
      case 'EDIFICACIONES': return 'bg-indigo-100 text-indigo-800 border-indigo-200'
      case 'DEPORTES_EDUCACION': return 'bg-red-100 text-red-800 border-red-200'
