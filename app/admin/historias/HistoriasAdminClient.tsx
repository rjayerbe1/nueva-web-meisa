...
interface HistoriasAdminClientProps {
  initialHistorias: HistoriaProyecto[]
  initialProyectosSinHistoria: any[]
}

export default function HistoriasAdminClient({
  initialHistorias,
  initialProyectosSinHistoria,
}: HistoriasAdminClientProps) {
  const [historias, setHistorias] = useState<HistoriaProyecto[]>(initialHistorias)
  const [proyectosSinHistoria] = useState(initialProyectosSinHistoria)

  const refetchHistorias = async () => {
    try {
