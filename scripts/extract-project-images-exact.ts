...
async function extractProjectImagesExact() {
  ...
  try {
      ...
      for (const proyecto of proyectos) {
        ...
        if (sliderEncontrado) {
          ...
          for (const slide of slides) {
            ...
            if (!imagenInfo) {
              const [directAttachments] = await connection.execute<any[]>(
                `SELECT 
                   ID,
                   post_title as imagen_titulo,
                   guid as imagen_url,
    ...
    for (const [categoria, proyectos] of Object.entries(resultados)) {
      ...
      for (const [nombreProyecto, datos] of Object.entries(proyectos as any)) {
        const datosProyecto = datos as any
        if (datosProyecto.slider_id) {
          console.log(`   ✅ ${nombreProyecto}: ${datosProyecto.total_imagenes} imágenes`)
          totalProyectosEncontrados++
          totalImagenesEncontradas += datosProyecto.total_imagenes
        } else {
